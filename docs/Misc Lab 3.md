...
1. **排序调色板**：将 PLTE 中的所有颜色按**亮度 (Luminance)** 排序
   - 亮度公式：$L = 0.299R + 0.587G + 0.114B$
   - 排序后相邻的颜色对人眼几乎无法区分

2. **嵌入秘密数据**（编码过程）：
   - 遍历每个像素，获取其调色板索引
   - 在亮度排序序列中找到该颜色的位置 $pos$
   - 检查 $pos$ 的 LSB 是否匹配要嵌入的秘密比特
   - 如果不匹配，将像素索引改为 $pos \oplus 1$（与相邻颜色交换）
   - 因为相邻颜色几乎相同，视觉上无差异

3. **提取秘密数据**（解码过程）：
   - 按亮度排序调色板
   - 对每个像素，找到其颜色在排序序列中的位置
   - 该位置的 **LSB** 即是隐藏的 1 bit
   - 按光栅顺序收集所有比特，转换为字节
